...
def _table_exists(table_name: str) -> bool:
    """Check if a table already exists (may have been created by Base.metadata.create_all)."""
    bind = op.get_bind()
...
def upgrade() -> None:
    ...
    if not _table_exists('social_connections'):
    ...
    if not _table_exists('onboarding_progress'):
    ...
    # Add profile_type column to users table (if not already added by create_all)
    if not _column_exists('users', 'profile_type'):
        with op.batch_alter_table('users', schema=None) as batch_op:
            batch_op.add_column(sa.Column('profile_type', sa.String(), nullable=True))

    # RBAC audit_log table (may already exist from Base.metadata.create_all)
    ...
    # RBAC access_logs table (may already exist from Base.metadata.create_all)
    if not _table_exists('access_logs'):
...
def downgrade() -> None:
    ...
    if _column_exists('users', 'profile_type'):
        with op.batch_alter_table('users', schema=None) as batch_op:
            batch_op.drop_column('profile_type')

    if _table_exists('onboarding_progress'):
        with op.batch_alter_table('onboarding_progress', schema=None) as batch_op:
    ...
    if _table_exists('social_connections'):
        with op.batch_alter_table('social_connections', schema=None) as batch_op:
